...
class LSTMParams(NamedTuple):
    layers: tuple[LSTM_layer, ...]
    num_layers: int
    hidden_size: int

def convert_lstm_params(lstm: EncodecLSTM) -> LSTMParams:
    return LSTMParams((
            LSTM_layer(pt2jax(lstm.lstm.weight_hh_l0), pt2jax(lstm.lstm.weight_ih_l0), pt2jax(lstm.lstm.bias_hh_l0), pt2jax(lstm.lstm.bias_ih_l0)), 
...
def forward_lstm(params: LSTMParams, inputs: Array) -> Array:
    params_layers, _, hidden_size = params
    ...
    for idx in range(len_):
        input_ = inputs[:,:,idx]
        for layer_n, param in enumerate(params_layers):
