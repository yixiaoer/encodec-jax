...
from encodec.lstm import LSTMParams, convert_lstm_parms, forward_lstm
from encodec.resnet import ResnetParams, convert_resnet_parms, forward_resnet

EncoderParams = list[Conv1dParams | ResnetParams | str | LSTMParams]

def convert_encoder(encoder: EncodecEncoder) -> EncoderParams:
    params: EncoderParams = []
    for layer in encoder.layers:
        if isinstance(layer, EncodecConv1d):
            params.append(convert_conv1d_parms(layer))
        elif isinstance(layer, EncodecResnetBlock):
            params.append(convert_resnet_parms(layer))
        elif isinstance(layer, EncodecLSTM):
            params.append(convert_lstm_parms(layer))
...
def test_forward_encoder(model: EncodecModel) -> None:
    ...
    encoder_params = convert_encoder(encoder)
